...
> 由于我在使用linux system的时候，不想安装密码随机工具，所以有了这个Repo；
> 如果你和我一样，你也可以随意使用这个项目，虽然这个项目实际上非常的简单；
> 正常情况下，如果使用的 Mac 或者 Windows，这同样可以使用； 如果不行，你可以自行修正或者给我提交一个issuse；
> 你只需要将 main.py文件中的内容完整的粘贴到你的终端配置文件中即可；
> 完成配置之后，还需要重启终端:
...
* **场景一：仅生成密码，不保存**
  ...
  randpass
  ```
  **效果**：终端会输出一个由颜色高亮的安全随机密码，并由于没有提供文件名，系统会用黄色的 `[WARN]` 提示你密码没有被保存
...
randpass() { ... }
```
* **含义**：定义一个名为 `randpass` 的 Shell 函数，在这个花括号 `{}` 里的内容，就是当敲击 `randpass` 时执行的所有指令
